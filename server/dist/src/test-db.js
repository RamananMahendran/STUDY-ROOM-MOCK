...
import { createProblem, deleteProblem, getAllProblems, getProblemById, updateProblem, } from './modules/practice/problemController.js';
...
const runController = async (handler, requestData = {}) => {
...
async function validateRequiredSchema() {
  ...
  `;
    ...
    assert(missingTables.length === 0, `Missing table(s): ${missingTables.join(', ')}. Run Prisma schema sync before testing the API.`);
  ...
  `;
    ...
    const missingColumns = Object.entries(requiredSchema).flatMap(([table, columns]) => (columns
        .filter((column) => !columnsByTable[table]?.has(column))
        .map((column) => `${table}.${column}`)));
    assert(missingColumns.length === 0, `Missing column(s): ${missingColumns.join(', ')}. Run Prisma schema sync before testing the API.`);
    console.log('PASS: Required server tables and columns are present.');
}
async function runAppFeatureTest() {
...
async function runAuthFeatureTest(testEmail, testUsername) {
    ...
    await protect(protectedRequest, protectedResponse, (error) => {
        if (error)
            throw error;
    });
...
async function runProblemsFeatureTest(timestamp) {
...
async function cleanup(testEmail, timestamp) {
    const tableRows = await prisma.$queryRaw `
    SELECT table_name
    FROM information_schema.tables
    WHERE table_schema = 'public'
...
async function runFullBackendDiagnostic() {

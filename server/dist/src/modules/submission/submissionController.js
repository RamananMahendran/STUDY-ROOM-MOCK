...
const normalizeOutput = (output, expected) => {
...
const processSubmissionInBackground = async (submissionId, code, languageId, testCases, submittedFrom, pairSessionId, userId) => {
...
export const submitProblem = async (req, res) => {
    try {
        ...
        processSubmissionInBackground(submission.id, code, languageId, testCases, submittedFrom, pairSessionId, userId);
...
export const getUserSubmissions = async (req, res) => {
    try {
        ...
        const submissions = await submissionModel.getUserSubmissions(userId, problemId ? parseInt(problemId) : undefined, parseInt(limit));
        return res.json({
            success: true,
            data: submissions,
        });
...
export const getUserSubmissionsPaginated = async (req, res) => {
    try {
        const userId = req.user?.id || 1;
...
export const getLatestPairSubmission = async (req, res) => {
    try {
        ...
        const submission = await submissionModel.getLatestPairSubmission(pairSessionId);
        return res.json({
            success: true,
            data: submission,
        });
...
export const getPairSessionSubmissions = async (req, res) => {
    try {
        const { pairSessionId } = req.params;
        const { limit = 50 } = req.query;
        const submissions = await submissionModel.getPairSessionSubmissions(pairSessionId, parseInt(limit));
        return res.json({
            success: true,
            data: submissions,
        });

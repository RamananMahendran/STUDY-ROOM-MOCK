...
export const joinSession = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const guestId = (req as any).user?.id || 2;
...
export const submitPairCode = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const submissionResult = await submitCodeFromPair(
      session.id,
      roomCode,
      userId,
      code,
...
export const updateCode = async (req: Request, res: Response): Promise<any> => {
  try {
    const roomCode = getParamString(req.params.roomCode);
    const { code } = req.body;
...
export const getPairSubmissions = async (req: Request, res: Response): Promise<any> => {
  try {
    const roomCode = getParamString(req.params.roomCode);

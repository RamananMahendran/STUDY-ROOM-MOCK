...
const isProUser = async (userId: number): Promise<boolean> => {
...
const selectRandomProblems = async (difficulty: string, count: number = 3) => {
  let whereClause: any = {};
  
  if (difficulty === 'easy') whereClause = { difficulty: 'easy' };
  else if (difficulty === 'medium') whereClause = { difficulty: 'medium' };
  else if (difficulty === 'hard') whereClause = { difficulty: 'hard' };
  else if (difficulty === 'mixed') whereClause = {};
...
export const getInterviewDetails = async (req: Request, res: Response): Promise<any> => {
  try {
...
export const submitProblemSolution = async (req: Request, res: Response): Promise<any> => {
  try {
    const id = getParamString(req.params.id);
...
export const completeInterview = async (req: Request, res: Response): Promise<any> => {
  try {
    const id = getParamString(req.params.id);
    const userId = (req as any).user?.id || 1;
    ...
    const difficultyWeakness: Record<string, number> = {};
    const topicWeakness: Record<string, number> = {};
...
export const getInterviewStatus = async (req: Request, res: Response): Promise<any> => {
  try {
    const id = getParamString(req.params.id);
    const userId = (req as any).user?.id || 1;
...
export const getInterviewHistory = async (req: Request, res: Response): Promise<any> => {
  try {
    const userId = (req as any).user?.id || 1;
...
export const getLeaderboard = async (req: Request, res: Response): Promise<any> => {
  try {
    ...
    const currentUserEntry = leaderboardData.find((entry: any) => entry.userId === userId);
    const currentUserRank = currentUserEntry 
      ? leaderboardData.findIndex((entry: any) => entry.userId === userId) + 1 
      : null;
